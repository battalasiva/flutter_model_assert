...
### 🚀 Major Update – Builder Inside Package

This release introduces a **stable, build-runner–based architecture**
to reliably detect backend API type mismatches.

### ✨ Added
- `@ModelAssert()` annotation for models
- Build-time extraction of model field paths and types
- Runtime validation API to check JSON before parsing
- Support for:
  - nested objects
  - lists
  - nested lists
- Clear error reports with:
  - exact field path
  - expected type
  - received backend value

### 🧠 Developer Experience
- One annotation per model
- One build command
- One-line runtime validation
- No schema files
- No dummy constructors

### 🛑 Prevents
- `double is not a subtype of int`
- `String is not a subtype of int?`
- Silent backend contract regressions

### ⚠️ Notes
- Requires `build_runner`
- Intended for debug / QA / staging
- Models must be updated manually after detection
